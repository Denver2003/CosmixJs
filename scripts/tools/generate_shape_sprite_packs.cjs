...
const CONFIG_PATH = path.join(ROOT, "scripts", "config.js");
const OUTPUT_DIR = path.join(ROOT, "assets", "sprite_packs");
...
function parseColors() {
  ...
  const match = raw.match(/export const COLORS\s*=\s*\[([\s\S]*?)\]/);
  if (!match) {
    throw new Error("COLORS not found in config.js");
  }
  const body = match[1];
  const colors = [];
  ...
  while ((m = regex.exec(body))) {
    const value = m[1];
    colors.push(value.startsWith("#") ? value : `#${value}`);
  }
  return colors;
}
...
function drawShape(img, shape, cellX, cellY, color) {
  const cx = cellX + CELL_SIZE / 2;
  const cy = cellY + CELL_SIZE / 2;
  const strokeColor = color;
  const fillColor = { ...color, a: FILL_ALPHA };
  ...
  if (shape === "rectangle") {
    ...
    drawRect(img, x, y, Math.round(w), Math.round(h), fillColor);
    drawRectOutline(img, x, y, Math.round(w), Math.round(h), strokeColor);
    return;
  }
  if (shape === "square") {
    ...
    drawRect(img, x, y, Math.round(w), Math.round(w), fillColor);
    drawRectOutline(img, x, y, Math.round(w), Math.round(w), strokeColor);
    return;
  }
  if (shape === "triangle") {
    ...
    fillPolygon(img, points, fillColor);
    drawPolygonStroke(img, points, strokeColor);
    return;
  }
  if (shape === "circle") {
    const r = CELL_SIZE * 0.28;
    fillEllipse(img, cx, cy, r, r, fillColor);
    drawEllipseStroke(img, cx, cy, r, r, strokeColor);
    return;
  }
  if (shape === "diamond") {
  ...
  if (shape === "oval") {
    ...
    fillEllipse(img, cx, cy, rx, ry, fillColor);
    drawEllipseStroke(img, cx, cy, rx, ry, strokeColor);
    return;
  }
  if (shape === "pentagon") {
...
function main() {
  ...
  for (const hex of colors) {
    const color = hexToRgba(hex, 1);
    const img = createImage(ATLAS_WIDTH, ATLAS_HEIGHT);
    SHAPES.forEach((shape, i) => {
      const col = i % COLS;
      const row = Math.floor(i / COLS);
      const cellX = col * CELL_SIZE;
      const cellY = row * CELL_SIZE;
      drawShape(img, shape, cellX, cellY, color);
    });
    const dirName = hex.replace("#", "");
    const outDir = path.join(OUTPUT_DIR, dirName);
    const outPath = path.join(outDir, "atlas.png");
    writePNG(outPath, ATLAS_WIDTH, ATLAS_HEIGHT, img.buffer);
  }
}
