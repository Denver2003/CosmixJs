...
LevelLoader::LevelLoader(){
	...
	TESTLOG("LevelLoader::LevelLoader()");
...
LevelLoader * LevelLoader::getInstance(){
    if (!glLevelLoader) {
		TESTLOG("LevelLoader::getInstance: create new instance"); 
...
LevelPack * LevelLoader::getLevelPack(string name){
    ...
    if (it != levelPacks.end()) {
		TESTLOG("LevelLoader::getLevelPack: level pack is founded(%s)",name.c_str());
        return it->second;
    }
	TESTLOG("LevelLoader::getLevelPack: level pack not found %s",name.c_str());
    return NULL;
}

void LevelLoader::loadLevelPacksFromFile()
{
    TESTLOG("LevelLoader::loadLevelPacksFromFile()");
...
vector<Level *> LevelLoader::loadLevels(lpCCDICTONARY dictLevels)
{
    ...
    if (dictLevels) {
        ...
        while ( dictLevel ) {
            ...
            CCArray * arrayShopInfo = ARRAY_FROM_DICT(dictLevel, "infos");
            level->infos = loadInfos( arrayShopInfo );
 ...
 vector<Stage *> LevelLoader::loadStages(lpCCDICTONARY dictLevel, Level * level){
	 vector<Stage *> stages;
		...
		while(dictStage){
            ...
            int minFigures          = INT_FROM_DICT( dictStage, "minFigures" );

			if(stars.size() == 3 ){
				Stage * stage = new Stage(stageLabel,waitingTime,points,stars[0],stars[1],stars[2],rotateAngle,bonusTime,minFigures);
		...
		if(bonusNames){
				...
				for (unsigned int i = 0; i < bonusNames->count(); i++) {
					...
					if (type != -1) {
                        ...
                        string info = STR_FROM_DICT(dictBonus, "info");
                        bonus->info = info;
...
vector<vector<string> > LevelLoader::loadInfos(CCArray * dictInfos)
{
    vector<vector<string> > _infos;
    if (dictInfos) {
        for (int i = 0; i < dictInfos->count(); i++) {
            vector<string> info;
            CCArray * arInfo = dynamic_cast<CCArray*>( dictInfos->objectAtIndex(i));
            if (arInfo) {
                for (int j = 0; j < arInfo->count(); j++) {
                    CCString * value = dynamic_cast<CCString*>( arInfo->objectAtIndex(j));
                    if (value) {
                        info.push_back(value->m_sString);
                    }
                }
            }
...
void LevelLoader::loadLevelData(){
    ...
    return;
	/*TESTLOG("LevelLoader::loadLevelData()");
    
	string fullPath = CCFileUtils::fullPathFromRelativePath("levels.plist");
    TESTLOG("LevelLoader::loadLevelData: path=%s",fullPath.c_str());
...
Stage * LevelLoader::getCurStage(){
    if (getCurLevelPack()) {
        ...
        if (!lvl) {
			TESTLOG("LevelLoader::getCurStage can't find current level!");
            return NULL;
        }
        if (curStageIndex >=0 && curStageIndex < lvl->stages.size() ) {
            //TESTLOG("LevelLoader::getCurStage get stage number: %i",curStageIndex);
			return lvl->stages[ curStageIndex ];
        }
		
		TESTLOG("LevelLoader::getCurStage wrong index = %i",curStageIndex);
        return NULL;
    }
    return NULL;
}

void LevelLoader::setStageFiguresCleared(int points)
{
    stageFiguresCleared = points;
    GameScene::gameScene->levelStatusUpdate();
}

void LevelLoader::addStageFiguresCleared(int points)
{
    stageFiguresCleared += points;
    GameScene::gameScene->levelStatusUpdate();
}
...
void LevelLoader::initNewLevel(){
    ...
    setStageFiguresCleared(0);
	TESTLOGFLIGHT("LevelLoader::initNewLevel");
}
//------------------------
// if return true - next stage success - if return false - no next stage! level complete
bool LevelLoader::toNextStage(){
	TESTLOG("LevelLoader::toNextStage");
    Level * lvl = getCurLevel();
    if (!lvl) {
		TESTLOG("LevelLoader::toNextStage not found current level");
        return false;
    }
	...
	TESTLOG("LevelLoader::toNextStage curStageIndex = %i, curStageNumber = %i level.stages.size()= %i ",
    ...
    if (curStageIndex >= lvl->stages.size() ) {
        ...
        /// create new stage!!!!!!!!!!!!
        ///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		TESTLOG("LevelLoader::toNextStage create new stage! lastIndex = %i", prevStageIndex);
		if( (prevStageIndex >=0 ) && ( prevStageIndex < lvl->stages.size())){
			...
			if( currStage ){
				...
				if( newStage ){
					...
					TESTLOG("LevelLoader::toNextStage creating new stage = OK");
					return true;
				}
				TESTLOG("LevelLoader::toNextStage error of creating newStage");
				return false;
			}
			TESTLOG("LevelLoader::toNextStage error of getting currStage");
			return false;
		
		}else{
			
			TESTLOG("LevelLoader::toNextStage error prevStageIndex = %i", prevStageIndex);
		}
...
bool LevelLoader::checkToNextStage( ){
	...
	if(!stg){
		TESTLOG( "checkToNextStage: NOT FOUND CURRENT STAGE!" );
		return false;
	}
    if (stg) {
        if ( stageFiguresCleared >= stg->points ) {
			TESTLOG("LevelLoader::checkToNextStage go to next level! stageFiguresCleared = %i,  stg->points = %i"
...
bool LevelLoader::readyToNextStage( ){
    ...
    if (stg) {
        if ( stageFiguresCleared >= stg->points ) {
			TESTLOG("LevelLoader::readyToNextStage ready to go to next level! stageFiguresCleared = %i,  stg->points = %i"
				,stageFiguresCleared, stg->points);
...
int LevelLoader::getStars(){
    Stage * stg = getCurStage();
	if(!stg){
		TESTLOG("LevelLoader::getStars no stage error!!!!");
		return 0;
	}
    
   if (stageCurrentPoints >= stg->starThree) {
	   TESTLOG("LevelLoader::getStars 3");
       return 3;
   }else if( stageCurrentPoints >= stg->starTwo ){
	   TESTLOG("LevelLoader::getStars 2");
       return 2;
   }else if( stageCurrentPoints >= stg->starOne ){
	   TESTLOG("LevelLoader::getStars 1");
       return 1;
   }
   TESTLOG("LevelLoader::getStars 0");
   return 0;
}
...
bool LevelLoader::addCleared( int _count ){
    Stage * stg = getCurStage();
    if(!stg){
		TESTLOG( "addCleared: NOT FOUND CURRENT STAGE!" );
		return false;
	}
    addStageFiguresCleared(_count);
    
    GameScene::gameScene->figureCleared+=_count;
    
    if (stageFiguresCleared > stg->points) {
		
		TESTLOG("LevelLoader::addCleared go to next level! stageFiguresCleared = %i,  stg->points = %i"
				,stageFiguresCleared, stg->points);

        GameScene::gameScene->preNewStage();
        GameScene::gameScene->levelStatusBlink();
        checkToNextStage();
...
bool LevelLoader::addPoints( int _points ){
	...
	TESTLOG("LevelLoader::addPoints add %i points! total on stage: %i points",_points,stageCurrentPoints);
    return true;
}
