...
GameScene::GameScene()
{
    ...
    CCSize pixelSize = CCDirector::sharedDirector()->getWinSize();
    
	
        //iphone
       /* if ( pixelSize.width == 320  ) {
            fontSizeScore = 28;
            fontSizeFloatScore = 24;
			CCLOG("GameScene::GameScene() this is iPhone");
        ...
        //ipad
        if ( ( pixelSize.width == 768 ) && (pixelSize.height == 1024) ) {
            // iPad
            fontSizeScore = 40;
            fontSizeFloatScore = 36;
    ...
    levelLayer = CCLayer::create();
    addChild(levelLayer, 1);

	/*topLayer = CCLayer::create();
	addChild(topLayer, 2);
	*/
    /*levelLayer->runAction(
            CCRepeatForever::create(
                        
                        (CCSequence*)CCSequence::create(
                        CCMoveBy::create(0.2, ccp(3,1)),
                        CCMoveBy::create(0.2, ccp(-5,-3)),
                        CCMoveBy::create(0.2, ccp(4,3)),
                        CCMoveBy::create(0.2, ccp(-3,-3)),                            
                        CCMoveTo::create(0.2, ccp(0,0)),                                                        
                        NULL
                                                         )
...
//	loader = new LevelHelperLoader( string( level->scene + ".plhs" ).c_str());
    loader = new LevelHelperLoader( string( "level.plhs" ).c_str());
    initLevelHelper(loader);
    
	/*LHSprite * sp = LHSprite::batchSpriteWithName("type210001",typesBatch);
	//LHSprite * sp = loader->createBatchSpriteWithName( "type21", "newTypes", "newTypes.pshs" );
	if(sp){

		sp->setPosition(ccp(100,100));
		

		sp->prepareAnimationNamed("t21_walk","newTypes.pshs");
		sp->playAnimation();
	}

	topLayer->addChild(sp);
*/
        //rectGameOver.origin.y = endOfLevelBezier->
...
void GameScene::initLevelHelper( LevelHelperLoader * _loader )
{
	CCLog("GameScene::initLevelHelper");
    loader = _loader;
    
	loader->addObjectsToWorld(world, levelLayer);    
    ...
    if (tmpSprite) {
        ...
        //loader->removeSprite(tmpSprite); ///old version
...
{
	CCLog("void SetAnchorPosition(CCSprite * sprite, const CCPoint & point)");

    sprite->boundingBox();
    static CCSize winSize = sprite->boundingBox().size;;//CCDirector::sharedDirector()->getWinSize();
...
void GameScene::initStarsAndPlanets()
{
	
	CCLog("GameScene::initStarsAndPlanets()");

	CCPoint anchorPoint;

	
	LHSprite * sozvezdia = loader->spriteWithUniqueName("sozvezdia");
	if( sozvezdia ){
		anchorPoint = sozvezdia->getPosition();
		CCRotateBy * rotate = CCRotateBy::create( 60, 180 );
		sozvezdia->runAction( CCRepeatForever::create( rotate ));
	}else {
        return;
    }
		
    CCNode * centerNode = CCNode::create();
    centerNode->setPosition(anchorPoint);
    
	CCArray * tmpSprites =  loader->spritesWithTag( STARS );
	if( tmpSprites ){
		for(unsigned int i=0; i < tmpSprites->count(); i++ ){
			...
			if(sprite){
                ...
                //sprite->setPosition(ccp(pos.x - rect.size.width/2,pos.y - rect.size.height/2));
				//SetAnchorPosition( sprite, anchorPoint );
				//sprite->setAnchorPoint
				//CCRotateBy * rotate = CCRotateBy::create( 80, 180 );
				//sprite->runAction( CCRepeatForever::create( rotate ));
                
                
                //sprite->getParent()->removeChild(sprite, false );
                //centerNode->addChild(sprite);
                pos.x = (anchorPoint.x - pos.x)/rect.size.width;
                pos.y = (anchorPoint.y - pos.y)/rect.size.height;
    ...
    //sozvezdia->getParent()->addChild(starsNode, zorder);
    //centerNode->setAnchorPoint(ccp(0.5f,0.5f));
    //centerNode->setPosition(anchorPoint);
    
    //centerNode->runAction( CCRepeatForever::create( rotate ));
    //addChild(centerNode);
        ...
        if (fallingSprite->getBody()) {
            ...
            if (abs(vec.y - prevVelocityY) > 2) {
                ...
                if (unit->animState!=ANIM_STATE_BLINKING) {
					...
					//CCMotionStreak::streakWithFade( 100, 4, 
                }
            }
	...
	if( CFigureUnit::rectCollideWhithSprites( rectGameOver ) ){
        ...
        if (laserType != LASER_FIND) {
            ...
            if (laserSprite) {
				...
				// old version
                //laserSprite->startAnimationNamed( "laser_find",0,this,callfuncND_selector( GameScene::laserEndAnim ));
            }
        }
		...
		if( timeToGameOver < 0 ){
			...
			/// need to call Game Over Function
                ...
                if (laserSprite) {
                    ...
                    // old version
					//laserSprite->startAnimationNamed( "laser_kill" );
                }
			gameOver();
		}
	}
...
void GameScene::clearTemplatesFromLevel()
{
	...
	//loader->removeSpritesWithTag(TEMPLATE);
...
void GameScene::stageInitCollisionInit()
{
	...
	Level * lvl = LevelLoader::getInstance()->getCurLevel();
	if(!lvl){
        CCLOG("Level not found!!!!");
        return;
	}

	map<int,ccColor3B>::iterator it = lvl->colors.begin();
	for( ; it != lvl->colors.end(); it++){
		int tag = it->first;
...
//// NEED TO KILL THIS FUNCTIONS
void GameScene::figureCollideWithEOL(LHSprite * sprite)
{
	CCLog("GameScene::figureCollideWithEOL(LHSprite * sprite)");

	map<LHSprite *,int>::iterator it = gameOverCollisions.find(sprite);
	if( it == gameOverCollisions.end()){
		gameOverCollisions.insert(make_pair(sprite,1));
	}else{
		it->second++;
	}
...
//// NEED TO KILL THIS FUNCTIONS
void GameScene::figureUncollideWithEOL(LHSprite * sprite)
{
	CCLog("GameScene::figureUncollideWithEOL(LHSprite * sprite)");
	...
	if( it == gameOverCollisions.end()){
		...
		it->second--;
		if( it->second < 1 ){
			gameOverCollisions.erase(it);
		}
	}
}
//// NEED TO KILL THIS FUNCTIONS
void GameScene::figureEndOfLevelCollision(cocos2d::CCObject *object)
{
	CCLog("GameScene::figureEndOfLevelCollision(cocos2d::CCObject *object)");
    ...
    if( true ){
       ...
       if (figure!=NULL) {
        ...
        /*if (figure!=NULL) {
            ...
            if (figureSprite == NULL) {
                figureSprite = loader->newPhysicalBatchSpriteWithUniqueName(figure->getSpriteName_01());
            }
  
            
            figureSprite->getBody()->SetType(b2_staticBody);
			//figureSprite->getBody()->
            figureSprite->moveOnPathWithUniqueName("startPath", 0.7, false, false, false, 0, false, false, false  );
            figureSprite->registerNotifierOnPathEndPoints(this, callfuncN_selector(GameScene::figureEndOfPath));
            figureSprite->setTag(figure->tag);
            figureSprite->setColor(figure->color);
            figureSprite->startAnimationNamed( figure->getAnimWalkName());
            currentSprite = figureSprite;
