...
//----------------------------------------
Alien * AliensManager::createPhysicsAlien	(Alien * _childAlien)
{
	if( !_childAlien  )
		return NULL;
    
	Alien * alien = createPhysicsAlien( _childAlien->figure );
...
//----------------------------------------
Alien * AliensManager::createPhysicsAlien	(Figure * _figure)
{
	Alien * alien = new Alien(Game::getLoader(), _figure, false);
	if( alien ){
		aliensBySprites.insert(make_pair(alien->getSprite(), alien));
...
void AliensManager::preCalculate()
{
    ...
    for (; it!=physicsAliens.end(); it++) {
        ...
        if (it->first) {
			...
			CCLOG("AliensManager::preCalculate WRONG UNIT");
		}
    }
...
{
	if( !alien ){
		CCLOG("AliensManager::calcGroup WRONG UNIT1");
		return;
	}
    ...
    for (; it!=aliens.end(); it++) {
		...
		if(alien2){
			...
			CCLOG("AliensManager::calcGroup WRONG UNIT2");
		}
    }
...
void AliensManager::calculate()
{
    ...
    for (; it!=physicsAliens.end(); it++) {
        ...
        if (alien!=NULL) {
			...
			CCLOG(" AliensManager::calculate WRONG UNIT");
		}
    }
}
//----------------------------------------
        ...
        if (sprite!=NULL) {
            if (alien!=NULL) {
                ...
                if (alien->isNeedToDelete) {
                    ...
                    LHSprite * dieSprite = GameScene::gameScene->loader->createBatchSpriteWithUniqueName(alien->figure->getSpriteName());
					...
					GameScene::gameScene->scoreHitAtPosition( sprite->getPosition(), alien->countCollided );
